...
module.exports = {
	...
	run: async (client, interaction) => {
		...
		.setColor('WHITE')
		.addFields(
			{ name: 'Status', value: ':bar_chart: waiting for community feedback. please vote.' }
		)
		

		try {
			let suggestedMsg = await suggestionChannel.send({ embeds: [suggestionEmbed] });
			suggestedMsg.react('<:yesyesyesy:908293466579152907>');
			suggestedMsg.react('<:nononon:908293430260695080>');
			interaction.reply({ content: `suggestion submitted in ${suggestionChannel}`, ephemeral: true });
		} catch (err) {
			console.error(err);
			interaction.reply({
