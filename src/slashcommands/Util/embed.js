...
const run = (client, interaction) => {
	...
	try {
		...
		if(err) {
			console.error(err);
			interaction.reply({ content: 'Failed to send your embed! Please check if you have provided appropriate color name/hex and URL(s)', ephemeral: true })
		}
	}
}
...
module.exports = {
	...
	options: [
		...
		{
			name: 'color',
			description: 'color of the embed. You can pass in hex code, color name or RANDOM',
			type: 'STRING',
			required: false
		},
		{
