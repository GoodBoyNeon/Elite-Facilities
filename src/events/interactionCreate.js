...
const handleButtons = (bot, interaction) => {
	...
	if(!buttonId) return interaction.reply({content: 'button not found', ephemeral: true})
...
const handleSlashCommands = (bot, interaction) => {
	...
	if(!interaction.inGuild()) return interaction.reply("This command can only be executed in a server");
